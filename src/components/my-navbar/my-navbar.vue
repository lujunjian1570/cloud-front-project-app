...
export default {
  ...
  computed: {
    ...
    // 返回箭头的颜色
    backIconColor() {
      return this.theme.navbarColor
    },
    // 返回文字的样式
    backTextStyle() {
      return {
        color: this.theme.navbarColor
      }
    },
    // 标题的颜色
    titleColor() {
    ...
    background() {
      return {
        background: this.theme.navbarBackgroundColor
      }
    },
    // title
    updateTitle() {
      // tabBarList底部导航列表
      const list = this.theme.platformThemeIconList
      let text = this.title
      if (list) {
        for (let i = 0; i < list.length; i++) {
          if (list[i].pagePath == this.pageUrl || list[i].pagePath == '/' + this.pageUrl) {
            text = list[i].iconText
            break
          }
        }
        return text
