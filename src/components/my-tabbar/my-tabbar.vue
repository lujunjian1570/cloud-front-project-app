...
export default {
  ...
  computed: {
    ...
    // 路径配置
    list() {
      return this.theme.platformThemeIconList
    },
    // 整个tabbar的背景颜色
    bgColor() {
      return this.theme.tabbarBackgroundColor
    },
    // 未激活时的颜色
    inactiveColor() {
      return this.theme.tabbarColor
    },
    // 激活时的演示，包括字体图标，提示文字等的演示
    activeColor() {
      return this.theme.tabbarSelectColor
    },
    elIconPath() {
      return (index) => {
        ...
        if (pagePath) {
          if (pagePath == this.pageUrl || pagePath == '/' + this.pageUrl) {
            return this.list[index].selectedIconPathUrl
          } else {
            return this.list[index].iconPathUrl
          }
        } else {
          // 普通方案中，索引等于v-model值时，即为激活项
          return index == this.value ? this.list[index].selectedIconPathUrl : this.list[index].iconPathUrl
        }
      }
    },
    elColor() {
